 ...
 * ```
 *
 * Face tokens: `none` | `eyes` | `glow` | `hybrid` | `classichybrid` | `trapezoid` | `eporo` | `fusion` | `fusionglow` | `fusioneyes` | `droid` | `glyph` (default: `hybrid`)
 *
 * Also:
 * ```
    ...
    override fun onReceive(context: Context, intent: Intent?) {
        when (intent?.action) {
            ACTION_SET -> {
                ...
                if (!ok) {
                    Log.w(TAG, "Unknown face '$raw' — use none|eyes|glow|hybrid|classichybrid|trapezoid|eporo|fusion|fusionglow|fusioneyes|droid|glyph")
                } else {
