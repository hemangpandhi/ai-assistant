...
**Branch posture:** product behavior still matches `origin/master` for the agent / VHAL / STT / LLM core. UI/UX, Compose face/overlay, and host wiring live in additive **packages** under the single `:app` module so PRs against master keep agent paths identical (no “new” agent classes). Policy details: [docs/architecture/refactor_extension_policy.md](architecture/refactor_extension_policy.md).

---

## Single `:app` module & how this branch sits on `master`

One Gradle module (`:app`). Ownership is by path/package so allowlisted agent Kotlin stays at the same `app/src/main/java/...` paths as `origin/master`.

| Path / package | Owns | Relation to `origin/master` |
|---|---|---|
| **Allowlisted `app/src/main/java/com/tcs/vehicleassistant/...`** | Master-owned Kotlin (orchestrator, VIS session, VHAL handlers, LiteRT, wake word, DI `appModule`, …) + shared agent `res` | **Byte-identical** (enforced by `MasterOwnedTreeContractTest`) |
| **`.../assistant/`** | UI/UX host: Compose session, `UiUx*` services, runtime bootstrap | Additive only |
| **`com/assistant/ui/`** | Compose face, overlay, immersive/gallery chrome, `AssistantBackend` / host contracts | UI presentation |
| **`com/assistant/api/`** | Host-neutral ports (`LlmSessionPort`, `ToolCatalog`) | Thin API boundary |
| **`:app` shell** | `AndroidManifest.xml`, assets, resources, local AARs, unit tests | May add UI/UX entries |

```mermaid
flowchart LR
  subgraph APK[":app"]
    MAN[AndroidManifest + assets/res]
    CORE[Agent under app/src/main/java — matches origin/master]
    UIUX[assistant/ UiUx host + session]
    FACE[com/assistant/ui Compose face]
    PORTS[com/assistant/api ports]
  end
...
1. **`UiUxVehicleApplication`** — loads Koin `appModule` (+ UX wiring).
2. **`UiUxAssistantVoiceInteractionService`** — system-bound VIS; starts wake-word / Compose hand-off.
3. **`UiUxAssistantSessionService`** — chooses Compose vs XML session; referenced from `voice_interaction_service.xml`.
4. Legacy master types (`AssistantVoiceInteractionService`, `VehicleAgentService`, `AssistantSession`, …) remain under `app/src/main/java` for parity and XML fallback.

### Syncing with `origin/master`

1. Fetch `origin/master`.
2. Copy / verify master Kotlin into the same `app/src/main/java/...` paths.
3. Merge or rebase for docs/tests/resources; keep UX in `assistant/` / `com/assistant/` packages.
4. Do not move agent Kotlin into a separate Gradle module (breaks clean PRs vs master).
