...
Master-owned Kotlin lives under **`app/src/main/java/com/tcs/vehicleassistant/`**
(same paths as `origin/master`) and must remain byte-identical for the allowlist.
UI/UX / TTFR extensions live in **`.../assistant/`**, **`com/assistant/ui/`**, and
**`com/assistant/api/`** inside `:app`. Do not add extension classes into
master-owned agent packages, and do not split agent Kotlin into a separate Gradle module.
...
| Compose / UI in `com/assistant/ui` | Patching XML session logic into the same class forever |
