...
| Module | Role |
|--------|------|
| `com/assistant/ui` (in `:app`) | Contracts (`AssistantBackend`, `AssistantHost`, events/models), face UI, overlay, demo backend, STT/TTS. |
...
1. New application module depends on `:assistant-ui`.
...
- Do not import `VehicleViewModel`, `DrivingUxState`, or `DesignAppShell` from `:assistant-ui`.
- Cabin facts cross the boundary only as `AssistantCabinContext` strings/numbers.
- Theme via `AssistantTheme` inside the assistant-ui module; host OEM theme is optional.
