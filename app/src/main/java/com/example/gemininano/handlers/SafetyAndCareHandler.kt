    ...
    override suspend fun execute(context: Context, toolCall: String, intentHandler: ((Intent) -> Unit)?): String {
        return when (handlerKey) {
            "handleDrowsyDriving", "handleDriverFatigue" -> {
                ...
                "I've lowered the temperature, increased the fan, and turned on the lights to help you stay alert. Please consider pulling over to rest."
            }
            "alertDriverDistraction" -> {
                "Please keep your eyes on the road and stay focused on driving."
            }
            "checkVehicleSecured" -> {
                "I've checked the vehicle. All doors and windows are closed and secured."
            }
            "checkTripReadiness" -> {
                "Vehicle diagnostics show everything is in order. Tire pressure, fluids, and battery are at optimal levels for your trip."
            }
            "improveRoadVisibility" -> {
                ...
                "I have activated the fog lights and optimized external lighting for better visibility."
            }
            else -> "System Error: Safety And Care Handler not recognized."
        }
    }
}
